...
export class IoTCClient implements IIoTCClient {
  ...
  async fetchTwin(): Promise<void> {
    this.twin = await util
      .promisify(this.deviceClient.getTwin)
      .bind(this.deviceClient)();
  }
  async uploadFile(
    fileName: string,
    filePath: string
  ...
  async connect(timeout: number = 30): Promise<any> {
    ...
    this.connectionstring = await promiseTimeout(
      this.register.bind(this, this.modelId),
      timeout * 1000
    );
    this.deviceClient = DeviceClient.fromConnectionString(
      this.connectionstring,
      await this.deviceProvisioning.getConnectionTransport(this.protocol)
    );
    ...
    this.deviceClient.on("disconnect", () => {
      if (connStatusEvent && connStatusEvent.callback) {
        (connStatusEvent.callback as ConnectionStatusCallback)(
          IOTC_CONNECTION_STATUS.DISCONNECTED
        );
      }
    ...
    this.deviceClient.on("connect", () => {
      if (connStatusEvent && connStatusEvent.callback) {
        (connStatusEvent.callback as ConnectionStatusCallback)(
          IOTC_CONNECTION_STATUS.CONNECTED
        );
      }
      this.connected = true;
    });
    ...
    try {
      await promiseTimeout(
        util.promisify(this.deviceClient.open).bind(this.deviceClient),
        timeout * 1000
      );
      this.connected = true;
      if (this.protocol !== DeviceTransport.HTTP) {
        this.twin = await promiseTimeout(
          util.promisify(this.deviceClient.getTwin).bind(this.deviceClient),
          timeout * 1000
        );
        this.subscribe();
      }
  ...
  private async register(): Promise<string> {
    ...
    if (this.authenticationType == IOTC_CONNECT.X509_CERT) {
      ...
      const x509Security = this.deviceProvisioning.generateX509SecurityClient(
        this.id,
        certificate
      );
      const registration = await this.deviceProvisioning.register(
        this.scopeId,
        this.protocol,
        x509Security
      );
      ...
      if (this.authenticationType == IOTC_CONNECT.SYMM_KEY) {
        sasKey = this.deviceProvisioning.computeDerivedKey(
      ...
      const sasSecurity = this.deviceProvisioning.generateSymKeySecurityClient(
        this.id,
        sasKey
      );
      const registration = await this.deviceProvisioning.register(
  ...
  async sendTelemetry(payload: any, properties?: any): Promise<void> {
    if (payload instanceof Array) {
      await this.deviceClient.sendEventBatch(
        payload.map((p) => new Message(JSON.stringify(p)))
      );
      return;
    }
    ...
    if (this.connected && this.twin) {
      ...
      this.deviceClient.on("message", (msg) => {
        if (msg.properties && msg.properties.propertyList) {
          const c2d = msg.properties.propertyList.find(
            (p) => p.key === "method-name"
          );
          if (c2d) {
  ...
  private async ackCommand(
    ...
    let resp = new DeviceMethodResponse(
      requestId,
      this.deviceClient._transport
    );
    ...
    if (this.protocol == DeviceTransport.MQTT) {
      this.deviceClient._transport.enableMethods((err) => {
        if (err) {
          ...
          (<any>this.deviceClient._transport)._mqtt.on(
            "message",
            (topic, payload) => {
      ...
      this.deviceClient._transport.enableMethods((err) => {
        (<any>(
          this.deviceClient._transport
        ))._deviceMethodClient._receiverLink.on("message", (msg) => {
          this.onCommandReceived({
